...
function buildWorkPackage(task, targetDir) {
  ...
  lines.push('```bash');
  const workerArg = task.claimedBy ? ` --worker ${task.claimedBy}` : '';
  lines.push(`yuva task done ${task.id}${workerArg} --summary "what you did"`);
  lines.push('```');
  lines.push('If you are blocked:');
  lines.push('```bash');
  lines.push(`yuva task fail ${task.id}${workerArg} --reason "why you are blocked"`);
  lines.push('```');
  lines.push('');
