...
- You MUST stay within the task scope — do not refactor unrelated code`;
  ...
  buildEnforcementSection(task, gates) {
    ...
    lines.push('');
    // The exact command (with the task id) lives in the per-task section at
    // the end of the work package. Naming the id here would put unique text
    // inside the cacheable frozen prefix and invalidate it for every task.
    lines.push('Run the `yuva task done` command given with your task below.');
  ...
  _getGitStatus() {
    try {
      ...
      return {
        ...
        staged: lines.filter(l => l[0] !== ' ' && l[0] !== '?' && l[0] !== '!' && l[0] !== 'U').map(l => l.slice(3)),
        unstaged: lines.filter(l => l[1] !== ' ' && l[1] !== '?').map(l => l.slice(3)),
      };
    } catch {
