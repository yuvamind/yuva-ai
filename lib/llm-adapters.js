...
const SHARED_CONFIG_FILES = new Set(['AGENTS.md', 'OLLAMA_INSTRUCTIONS.md']);
...
function getConfiguredLLMId(targetDir) {
  const projectConfig = readJSON(P.configFile(targetDir)) || {};
  const configured = projectConfig.llm || (
    projectConfig.tool && projectConfig.tool !== 'all' ? projectConfig.tool : null
  );
  return LLM_CONFIGS[configured] ? configured : null;
}

function hasDetectableConfig(targetDir, id, config) {
  if (!fileExists(path.join(targetDir, config.configFile))) return false;
  if (!SHARED_CONFIG_FILES.has(config.configFile)) return true;

  // AGENTS.md and OLLAMA_INSTRUCTIONS.md are intentionally reusable across
  // multiple platforms. They only identify a platform when project metadata
  // explicitly selects that platform; otherwise detection must stay silent.
  return getConfiguredLLMId(targetDir) === id;
}
...
function detectLLM(targetDir) {
  ...
  for (const id of priority) {
    ...
    if (config && hasDetectableConfig(targetDir, id, config)) {
      return { id, ...config };
    }
  }
  return null;
...
function detectAllLLMs(targetDir) {
  ...
  for (const [id, config] of Object.entries(LLM_CONFIGS)) {
    if (hasDetectableConfig(targetDir, id, config)) {
