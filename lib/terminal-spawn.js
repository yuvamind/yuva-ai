...
function buildSpawnSpec(platform, command, { title = 'Yuva', cwd = process.cwd(), terminal = null } = {}) {
  if (platform === 'win32') {
    ...
    return {
      cmd: 'cmd.exe',
      // `start` treats its first quoted argument as the window title. Passing
      // a title containing spaces through `cmd /c` is not reliable across
      // Windows command-line parsing layers; when it is split, `/D` leaks
      // through to the worker command and yuva reports it as an unknown
      // command. An explicit empty title makes the following arguments
      // unambiguous. Windows Terminal preserves the title in the branch above.
      args: ['/d', '/c', 'start', '', '/D', cwd, 'cmd.exe', '/d', '/k', command],
  ...
  // Quote the directory for the nested shell. Project paths can contain
  // apostrophes or shell metacharacters and must never become shell syntax.
  const shellQuote = value => `'${String(value).replace(/'/g, "'\\''")}'`;
  const inner = `cd ${shellQuote(cwd)} && ${command}; exec bash`;
  const attempts = [
