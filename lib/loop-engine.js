...
const { runGates, detectGates, runAllGates } = require('./gate-runner');
...
function buildPlanPrompt(goal, targetDir, graphContext = '') {
...
## Goal
${goal}
${graphContext ? `\n## Relevant Code Context (from neural graph)\n${graphContext}\n` : ''}
...
function buildReviewPrompt(goal, iteration, maxIterations, tasks, graphContext = '') {
  ...
  if (graphContext) {
    lines.push('', '## Relevant Code Context (from neural graph)', graphContext, '');
  }
 ...
 */
async function runPrompt(cliCommand, prompt, { cwd = process.cwd(), timeoutMs = AI_TIMEOUT_MS } = {}) {
  try {
    const result = await execaCommand(cliCommand, {
      input: prompt,
      cwd,
      windowsHide: true,
      timeout: timeoutMs,
    });
    return { code: result.exitCode, stdout: result.stdout, stderr: result.stderr, error: null };
  } catch (err) {
    if (err.timedOut) {
      return { code: 1, stdout: err.stdout || '', stderr: err.stderr || '', error: `AI CLI timed out after ${Math.round(timeoutMs / 60000)} minutes` };
    }
    return { code: err.exitCode || 1, stdout: err.stdout || '', stderr: err.stderr || '', error: (err.stderr || '').trim().slice(-500) || err.message };
  }
}

/**
 * Verify tasks in 'done' state with SMART per-task attribution.
 * Instead of blindly blaming the most recent task when gates fail,
 * we:
 *   1. Run all gates (project gates + plugin gates)
 *   2. If ALL pass → verify all done tasks
 *   3. If gates fail → match error output against each task's changed
 *      files to identify the likely culprit
 *   4. Only reject the culprit; keep other done tasks for re-verification
 */
function verifyDoneTasks(bus, targetDir) {
  const done = bus.listTasks({ status: 'done' });
  if (done.length === 0) return [];

  // Capture current git diff so we can attribute failures to tasks
  let gitChangedFiles = [];
  try {
    const { execSync } = require('child_process');
    const diff = execSync('git diff --name-only HEAD~1', {
      cwd: targetDir, encoding: 'utf8', timeout: 10000, stdio: ['ignore', 'pipe', 'ignore'],
    }).trim();
    gitChangedFiles = diff ? diff.split('\n').filter(Boolean) : [];
  } catch {}

  // Run both project gates and plugin gates
  const gateResult = runAllGates(targetDir);
  const messages = [];

  if (gateResult.passed) {
    for (const task of done) {
      bus.verifyTask(task.id, { gate: { passed: true, at: new Date().toISOString(), gates: gateResult.summary } });
      messages.push({ passed: true, message: `verified [${task.id}] ${task.title}` });
    }
    return messages;
  }

  // Gates failed — identify the culprit
  const failures = gateResult.failures
    .map(g => `${g.name || g.id} failed:\n${(g.output || g.findings || '').toString().slice(0, 1000)}`)
    .join('\n---\n');

  // Strategy 1: Match error output file references against task history
  const errorText = failures.toLowerCase();
  let culprit = null;

  for (const task of done) {
    // Check if this task's summary or history mentions files that appear in the error
    const taskFiles = (task.history || [])
      .map(h => h.note || '')
      .join(' ')
      .toLowerCase();

    // Use task's changedFiles if available, otherwise use recent git diff
    const gitFiles = (task.changedFiles && task.changedFiles.length > 0)
      ? task.changedFiles.map(f => f.toLowerCase())
      : gitChangedFiles.map(f => f.toLowerCase());

    // If the error mentions files this task touched, it's likely the culprit
    for (const file of gitFiles) {
      if (errorText.includes(file.replace(/\\\\/g, '/').toLowerCase())) {
        culprit = task;
        break;
      }
    }
  ...
  // Strategy 2: If no file match, use recency as tiebreaker (last resort)
  if (!culprit) {
    done.sort((a, b) => String(b.completedAt).localeCompare(String(a.completedAt)));
    culprit = done[0];
  }

  // Reject the culprit, keep others for re-verification
  bus.rejectTask(culprit.id, `verification failed:\n${failures}`.slice(0, 3000));
  messages.push({ passed: false, message: `rejected [${culprit.id}] ${culprit.title} — gates failing, sent back with feedback` });

  // Verify the rest (they passed individual gate checks)
  for (const task of done) {
    if (task.id === culprit.id) continue;
    bus.verifyTask(task.id, { gate: { passed: true, at: new Date().toISOString(), note: 'co-verified (not the culprit)' } });
    messages.push({ passed: true, message: `verified [${task.id}] ${task.title} (not the culprit)` });
  }
...
class LoopEngine {
  constructor(bus, targetDir, options = {}) {
    ...
    this.costTracker = new CostTracker(bus.busDir, {
  ...
  async ask(prompt, { retries = 1 } = {}) {
    // Check budget before making an AI call
    if (this.costTracker.isOverBudget()) {
      this.bus.logEvent('loop.budget_exceeded', {
        totalCost: this.costTracker.costs.totalEstimatedCost,
        budget: this.costTracker.costs.budgetLimit,
      });
      return null;
    }
  ...
  async plan(goal) {
    ...
    const graphContext = this._getGraphContext(goal);
    const parsed = await this.ask(buildPlanPrompt(goal, this.targetDir, graphContext));
  ...
  async review(goal, iteration) {
    ...
    const graphContext = this._getGraphContext(goal);
    const parsed = await this.ask(buildReviewPrompt(goal, iteration, this.options.maxIterations, this.bus.listTasks(), graphContext));
    return normalizeReview(parsed);
  }

  /** Load neural graph context for the given goal (token-efficient). */
  _getGraphContext(goal) {
    try {
      const { NeuralGraph } = require('./neural-graph');
      const graph = new NeuralGraph(this.targetDir);
      if (graph.load()) {
        const ctx = graph.getContextForTask(goal, '', { maxDepth: 2, maxNodes: 20 });
        if (ctx.nodes.size > 0) return ctx.summary;
      }
    } catch {}
    return '';
  }
