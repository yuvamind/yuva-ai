...
function scanProject(targetDir) {
  ...
  // Git info
  try {
    ctx.gitBranch = execSync('git rev-parse --abbrev-ref HEAD', { cwd: targetDir, encoding: 'utf8', timeout: 5000, stdio: ['ignore', 'pipe', 'ignore'] }).trim();
  } catch {}
  try {
    const status = execSync('git status --porcelain', { cwd: targetDir, encoding: 'utf8', timeout: 5000, stdio: ['ignore', 'pipe', 'ignore'] }).trim();
    ctx.gitStatus = status === '' ? 'clean' : 'dirty';
  } catch { ctx.gitStatus = 'not-a-repo'; }
  try {
    const log = execSync('git log --oneline -5', { cwd: targetDir, encoding: 'utf8', timeout: 5000, stdio: ['ignore', 'pipe', 'ignore'] }).trim();
  ...
  // Count source files using glob (cross-platform, no shell commands)
  try {
    const { collectSourceFiles } = require('./fs-utils');
    const files = collectSourceFiles(targetDir, {
      extensions: ['js', 'ts', 'jsx', 'tsx', 'py', 'go', 'rs', 'java', 'rb', 'php', 'dart'],
      maxDepth: 4,
    });
    ctx.totalFiles = Math.min(files.length, 500);
  } catch {}
...
function injectContext(prompt, targetDir, extraContext = {}) {
  ...
  for (const [key, value] of Object.entries(extraContext)) {
    result = result.replace(new RegExp(`\\{\\{${key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').toUpperCase()}\\}\\}`, 'g'), String(value));
  }
