...
function swarmSpawn(bus, flags, targetDir) {
  ...
  // Resolve which AI CLI to boot in each terminal: --cli flag > configured tool
  ...
  if (!cli) {
    ...
    cli = (config && TOOL_CLI[config.tool]) || null;
  }
  if (cli && cli.includes('"')) {
  ...
  if (cli) info(`AI CLI: ${cli}`);
  else warn('No AI CLI resolved — terminals will print work packages for manual AI sessions. Use --cli <command> to auto-boot an AI.');
...
function swarmCommand(args = []) {
  const { positional, flags } = parseFlags(args, { booleans: ['headless'] });
