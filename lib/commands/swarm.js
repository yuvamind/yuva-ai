...
function renderDashboard(bus, { verifications = [] } = {}) {
  ...
  try {
    ...
    if (graph.load()) {
      ...
      log(`Neural Graph: ${stats.totalNodes} nodes, ${stats.totalEdges} edges`, 'dim');
    }
  } catch {}

  // Show cost tracking
  try {
    const costTracker = new CostTracker(process.cwd());
    const costSummary = costTracker.getSummary();
    if (costSummary.totalCalls > 0) {
      log(`Cost: ${costSummary.totalCalls} calls, $${costSummary.totalEstimatedCost}`, 'dim');
    }
  } catch {}
