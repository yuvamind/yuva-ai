...
async function initCommand(options = {}) {
  ...
  if (dryRun) {
    ...
    log('   .yuva/agents.md (agent index)');
    if (all) {
  ...
  try {
    ...
    // 2. Create .yuva/config.json
    const configDir = P.configDir(targetDir);
    ensureDir(configDir);
    // .yuva/ is committed; only .yuva/run/ (task bus, sessions, caches) is not
    ...
    createdFiles.push('.yuva/config.json');

    // 3. Copy agents.md index
    const agentsIndexSrc = P.agentsIndex(templatePath);
    if (fileExists(agentsIndexSrc)) {
      fs.copyFileSync(agentsIndexSrc, path.join(configDir, 'agents.md'));
      createdFiles.push('.yuva/agents.md');
    }
    ...
    } catch (err) { /* graph build failed during init — non-critical */ }
