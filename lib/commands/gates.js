...
function gatesCommand(args = [], targetDir = process.cwd()) {
  const subcommand = args[0] || 'run';

  switch (subcommand) {
    case 'run':
      return finishGateCommand(runGates(targetDir));
    case 'list':
...
function finishGateCommand(result) {
...
function runGates(targetDir) {
  ...
  try {
    ...
    if (result.passed) {
      ...
      const failed = result.gates.filter(g => g.blocking);
      log(`\n❌ ${failed.length} gate(s) failed.\n`, 'red');
    }
    return result;
  } catch (err) {
    log(`Plugin gates failed: ${err.message}\n`, 'red');
    return { passed: false, gates: [], error: err.message };
  }
}
...
function listGates(targetDir) {
  ...
  log('  1. Create .yuva/gates/<name>.js');
  log('  2. Export: { name, description, severity, run(targetDir) => findings[] }');
  log('  3. Enable/disable in .yuva/config.json: { "pluginGates": { "<name>": true } }\n');
}
