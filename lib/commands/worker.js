...
function buildWorkerBootPrompt(role) {
  return [
    ...
    `3) If no task is available, run: yuva task list — if pending tasks exist for other reasons wait ~30 seconds, then go back to step 1.`,
    '4) After completing a task, go back to step 1 for the next one.',
    'Never stop the loop unless the user tells you to stop. Do not invent work outside the bus.',
