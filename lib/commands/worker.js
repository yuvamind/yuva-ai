...
const { execaCommand, execaCommandSync } = require('execa');
...
async function runCli(cliCommand, workPackage) {
  try {
    ...
    return { code: err.exitCode || 1, stdout: err.stdout || '', stderr: err.stderr || '', error: err.message };
  }
}

/** Return a failure message when the worker's CLI run did not succeed. */
function getWorkerRunFailure(result) {
  const cliResult = result.cliResult || null;
  if (result.success === false || (cliResult && cliResult.code !== 0)) {
    return (cliResult && (cliResult.error || cliResult.stderr))
      || result.error
      || `exit code ${(cliResult && cliResult.code) ?? 1}`;
  }
  return null;
}
  ...
  while (running) {
    ...
    if (useGitBranch) {
      ...
      result = await git.isolateTask(worker.id, task.id, async () => {
        ...
        return {
          success: r.code === 0,
          summary: `CLI exit code ${r.code}`,
          error: r.code === 0 ? null : (r.error || r.stderr || `CLI exit code ${r.code}`),
        };
      });
    } else {
      // No git isolation — run directly
      const r = await streamer.run(task.id, cliCommand, { cwd: process.cwd() });
      result = { success: r.code === 0, cliResult: r };
    }

    clearInterval(beat);

    // Check for a failed CLI run before any gate-based completion fallback.
    // A non-zero exit must never become a successful task merely because the
    // working tree happens to pass the gates.
    ...
    const runFailure = getWorkerRunFailure(result);
    if (runFailure) {
      error(`CLI failed: ${runFailure}`);
      bus.rejectTask(task.id, `worker CLI failed: ${runFailure}`);
      bus.heartbeat(worker.id, { status: 'idle', currentTask: null });
      if (flags.once) break;
      continue;
    }
    ...
    if (after && after.status === 'claimed') {
      ...
      if (gateResult.passed) {
        ...
        const failures = [
          ...
          ...gateResult.pluginGates.gates.filter(g => g.blocking).map(g => `plugin:${g.name}: ${g.findings.length} findings`),
        ].join('\n---\n');
...
function buildWorkerBootPrompt(role) {
  const { protectedFilesSummary } = require('../enforcement-rules');
  return [
    `You are a Yuva swarm WORKER with the role: ${role}.`,
    'Work in a strict loop:',
    `1) Run: yuva worker next --role ${role}`,
    '2) If it prints a work package, follow it EXACTLY — its agent instructions and enforcement rules are mandatory. Finish ONLY via the yuva task done command it specifies (quality gates run automatically; if they fail, fix and retry).',
    ...
    `PROTECTED FILES — NEVER delete, move, or empty: ${protectedFilesSummary()}. Never run yuva swarm clear or yuva session clear. (This is enforced automatically, not just a request.)`,
  ].join(' ');
}
...
function workerBoot(bus, flags) {
  ...
  if (cli) {
    ...
    // Pass prompt via stdin to avoid shell injection through the prompt string
    const result = execaCommandSync(cli, {
    ...
    if (result.failed) {
      error(`Could not launch "${cli}": ${result.shortMessage || `exit code ${result.exitCode}`}`);
...
module.exports.getWorkerRunFailure = getWorkerRunFailure;
