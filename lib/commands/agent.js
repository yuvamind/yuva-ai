...
function showAgent(name) {
  ...
  if (localFile) {
    const localPath = path.join(P.promptsDir(targetDir), localFile);
  ...
  const customPath = path.join(P.promptsDir(targetDir), `${name}agent.md`);
  ...
  const agentPath = path.join(pkgPath, 'template', '.yuva', 'prompts', fileName);
...
function listAgents() {
  ...
  for (const [name, desc] of Object.entries(AGENT_DESCRIPTIONS)) {
    const fileName = AGENT_MAP[name];
    const localPath = path.join(P.promptsDir(targetDir), fileName);
...
function orchestrate() {
  ...
  // Check session
  result.hasSession = fileExists(path.join(P.sessionDir(targetDir), 'state.md')) ||
    fileExists(path.join(targetDir, '.session', 'state.md'));
  ...
  // (set "mode": "solo" in .yuva/config.json to opt out)
