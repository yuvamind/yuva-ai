...
const { readJSON, fileExists, walkSourceFiles } = require('./fs-utils');
const P = require('./paths');

const BLOCKING_SEVERITIES = new Set(['error', 'high', 'critical']);

function isBlockingSeverity(severity) {
  return BLOCKING_SEVERITIES.has(String(severity || 'warning').toLowerCase());
}
...
const BUILTIN_RULES = {
  'no-console-log': {
    ...
    run(targetDir) {
      ...
      walkSourceFiles(targetDir, {
        ...
        dirs: ['src', 'lib', 'app', 'pages', 'api', 'server'],
    ...
    run(targetDir) {
      ...
      walkSourceFiles(targetDir, {
        ...
        callback: (filePath, content) => {
          const lines = content.split('\n');
          for (let i = 0; i < lines.length; i++) {
            const match = lines[i].match(/\b(?:TODO|FIXME|HACK|XXX)[:\s]+(.*)/);
            if (match) {
              findings.push({
                file: path.relative(targetDir, filePath),
                line: i + 1,
  ...
  'no-unused-deps': {
    ...
    run(targetDir) {
      ...
      walkSourceFiles(targetDir, {
        extensions: ['js', 'ts', 'jsx', 'tsx'],
        dirs: ['src', 'lib', 'app', 'pages', 'api'],
        callback: (_filePath, content) => { allSource.push(content); },
      });

      const combined = allSource.join('\n');
      for (const dep of Object.keys(pkg.dependencies)) {
        const depEscaped = dep.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const pattern = new RegExp(`(?:require\\s*\\(\\s*['"]${depEscaped}['"]|from\\s+['"]${depEscaped})`);
 ...
 * Gates are configured in .yuva/config.json:
 *   { "pluginGates": { "no-console-log": true, "no-todo-fixme": false } }
 *
 * Custom rules can be added in .yuva/gates/<name>.js
 */
function runPluginGates(targetDir) {
  ...
  for (const [ruleId, rule] of Object.entries(BUILTIN_RULES)) {
    ...
    results.push({
      ...
      blocking: findings.length > 0 && isBlockingSeverity(rule.severity),
      findings: findings.slice(0, 20), // cap
      totalFindings: findings.length,
    });
  }

  // Load custom rules from .yuva/gates/
  const gatesDir = P.gatesDir(targetDir);
      ...
      try {
        ...
        if (typeof rule.run === 'function') {
          ...
          results.push({
            ...
            blocking: Array.isArray(findings) && findings.length > 0 && isBlockingSeverity(rule.severity),
  ...
  return {
    passed: allPassed,
    gates: results,
    blockingFailures: results.filter(r => r.blocking),
  };
}
...
module.exports = { BUILTIN_RULES, runPluginGates, formatPluginGates, isBlockingSeverity };
