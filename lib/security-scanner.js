...
const { classifyFinding } = require('./decision-engine');
...
function scanDependencies(targetDir) {
  ...
  if (fileExists(path.join(targetDir, 'package.json'))) {
    try {
      const output = runJsonTool('npm', ['audit', '--json'], targetDir);
      if (output) {
  ...
  if (fileExists(path.join(targetDir, 'requirements.txt')) || fileExists(path.join(targetDir, 'pyproject.toml'))) {
    try {
      const output = runJsonTool('pip-audit', ['--format', 'json'], targetDir);
      if (output) {
        const audit = JSON.parse(output);
...
/** Run an optional JSON-producing audit tool without invoking a shell. */
function runJsonTool(command, args, cwd) {
  try {
    ...
    // npm audit and pip-audit use exit code 1 when vulnerabilities are found;
    // their JSON stdout is still useful and should be parsed.
    return err.stdout || '';
  }
}
...
function scanForSecrets(targetDir) {
  ...
  const codeExts = ['.js', '.ts', '.jsx', '.tsx', '.py', '.go', '.rs', '.java', '.rb'];
  const ignore = new Set(['node_modules', '.git', 'dist', 'build', '.yuva']);
  const isScannableFile = (name) => {
    const lowerName = name.toLowerCase();
    return lowerName === '.env'
      || lowerName.startsWith('.env.')
      || codeExts.includes(path.extname(lowerName));
  };
...
function gitignorePatternMatches(pattern, fileName) {
  ...
  const candidate = normalized.includes('/') ? target : path.posix.basename(target);
  let source = '';
...
function isGitignored(gitignore, fileName) {
  let ignored = false;
  for (const rawLine of gitignore.split(/\r?\n/)) {
    ...
    if (gitignorePatternMatches(pattern, fileName)) ignored = !negated;
  }
  return ignored;
}
...
function scanEnvExposure(targetDir) {
  ...
  for (const f of envFiles) {
    if (fileExists(path.join(targetDir, f)) && !isGitignored(gitignore, f)) {
      findings.push({
        severity: 'high',
        category: 'config',
