...
# Source of truth: AGENTS.md + .yuva/
CLAUDE.md
.claude/
GEMINI.md
...
.memory/
`;
// NOTE: .yuva/ is deliberately absent — it holds COMMITTED project config.
// Its runtime half is ignored via paths.ensureGitignore(), which adds the
// single entry .yuva/run/.
...
Finish only via the exact \`yuva task done\` command in the work package (it includes the worker identity; gates run automatically).
...
\`.yuva/\` (task bus + sessions), \`.session/\`, \`.yuva/\`, \`AGENTS.md\`,
...
5. Run \`yuva gate\` before declaring ANY work complete
6. NEVER touch the protected files listed above
...
function generateClaudeConfig(targetDir) {
  ...
  const settings = {
    ...
    hooks: {
      ...
      // Real-time enforcement: blocks edits/deletes to yuva's own
      // plumbing (task bus, session state, agent config) and the two
      // destructive "clear" commands, instead of only asking the AI
      // nicely in a prompt and checking after the fact.
      PreToolUse: [
        {
...
Run \`yuva gate\` before declaring work complete.
`;
...
Run \`yuva gate\` after writing tests.
`;
...
\`.yuva/prompts/<agentname>agent.md\`
