...
## Swarm Mode (multi-terminal orchestrator/worker)
If a \`.yuva/\` directory exists, this project is running in swarm mode:
- To pick up work in this terminal: \`yuva worker next --role <role>\`
  and follow the printed work package EXACTLY.
- Finish only via \`yuva task done <id> --summary "..."\` (gates run automatically).
- The orchestrator terminal runs \`yuva swarm start\` and verifies every result.
