...
function readSessionJSON(dir) {
  return JSON.parse(fs.readFileSync(path.join(dir, '.yuva', 'run', 'session', 'session.json'), 'utf8'));
}
...
describe('session CLI', () => {
  ...
  it('session clear removes session files', () => {
    const dir = makeTempDir();
    run('session start "Build login page"', dir);
    expect(fs.existsSync(path.join(dir, '.yuva', 'run', 'session'))).toBe(true);
    run('session clear', dir);
    expect(fs.existsSync(path.join(dir, '.yuva', 'run', 'session'))).toBe(false);
  });
});
